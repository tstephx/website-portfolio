...
```
Applied to a best-in-class partner program as competitive intelligence, then built a
101-requirement redesign spanning 20 countries that secured 15× the engineering investment.
```

Drops "competitor's franchise program" (which identifies the target to anyone in last-mile logistics) and replaces with "best-in-class partner program" — still signals initiative and competitive benchmarking without naming or narrowing the target. Change is live on `index.html`.
